...
return new class extends Migration
{
    ...
    public function down(): void
    {
        Schema::table('jobs', function (Blueprint $table) {
            ...
            $table->dropForeign(['company_region_id']);
            $table->dropForeign(['company_city_id']);
            $table->dropForeign(['company_neighborhood_id']);

            $table->dropColumn('region_id');
            $table->dropColumn('city_id');
            $table->dropColumn('neighborhood_id');
            $table->dropColumn('company_region_id');
            $table->dropColumn('company_city_id');
            $table->dropColumn('company_neighborhood_id');
        });
    }
};
